...
public class LeaveFormService {
    ...
    public LeaveForm createLeaveForm(LeaveForm form){
        LeaveForm savedForm = (LeaveForm) MybatisUtils.executeUpdate(SqlSession->{
            ...
            //2.增加一条流程数据，说明表单已提交，状态为complete
            ...
            //3.分情况创建其余流程数据
            //3.1 7级以下员工生成部门经理审批任务，请假时间大于36小时，还需生成总经理审批任务
        ...
        MybatisUtils.executeUpdate(SqlSession->{
            ...
            if(process.getIsLast() == 1){
                ...
                if(result.equals("approved")){
                    ...
                    String noticeContent1 = String.format("您的请假申请[%s-%s]%s%s已批准,审批意见:%s,审批流程交由上级继续审批."
                            ,employee.getTitle(),employee.getName()
                            ,sdf.format(form.getStartTime()),sdf.format(form.getEndTime())
                            ,reason);
                    ...
                    //消息2：通知总经理有新的审批任务
                    String noticeContent2 = String.format("%s-%s提请请假申请[%s-%s],请尽快审批."
                            ,employee.getTitle(),employee.getName()
                            ,sdf.format(form.getStartTime()),sdf.format(form.getEndTime()));
                    noticeDao.insert(new Notice(readyProcess.getOperatorId(),noticeContent2));

                    //消息3：通知部门经理（当前经办人）,员工的申请单你已批准,交由上级继续审批
                    String noticeContent3 = String.format("%s-%s提起请假申请[%s-%s]您已批准,审批意见:%s,审批流程交由上级继续审批."
                            ,employee.getTitle(),employee.getName()
                            ,sdf.format(form.getStartTime()),sdf.format(form.getEndTime())
                            ,reason);
